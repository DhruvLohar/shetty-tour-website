...
class DiscGeometry extends Geometry {
  constructor(steps = 4, radius = 1) {
    ...
    // Create a rounded rectangle instead of a circle
    ...
    // Helper function to add corner arc
    const addCornerArc = (cx, cy, startAngle, segments) => {
    ...
    // Top-right corner
    addCornerArc(width / 2 - cornerRadius, height / 2 - cornerRadius, 0, cornerSegments);
    // Top-left corner
    addCornerArc(-width / 2 + cornerRadius, height / 2 - cornerRadius, Math.PI / 2, cornerSegments);
    // Bottom-left corner
    addCornerArc(-width / 2 + cornerRadius, -height / 2 + cornerRadius, Math.PI, cornerSegments);
    // Bottom-right corner
    addCornerArc(width / 2 - cornerRadius, -height / 2 + cornerRadius, Math.PI * 1.5, cornerSegments);
    
    // Add vertices
    vertices.forEach(v => {
      this.addVertex(v.x, v.y, 0);
      this.lastVertex.uv[0] = (v.x / width) + 0.5;
      this.lastVertex.uv[1] = (v.y / height) + 0.5;
    });
    ...
    this.addFace(0, vertices.length, 1);
  }
}
    ...
    <>
      <div style={{ position: 'relative', width: '100%', height: '100%' }}>
        ...
        {activeItem && (
          <>
            ...
            <p className={`face-description ${isMoving ? 'inactive' : 'active'}`} dangerouslySetInnerHTML={{ __html: activeItem.description }} />
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-7xl w-[95vw]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold">{activeItem?.title}</DialogTitle>
            <DialogDescription dangerouslySetInnerHTML={{ __html: activeItem?.description || '' }} />
          </DialogHeader>
          <FleetCarousel 
